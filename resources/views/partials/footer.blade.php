    ...
    <div class="max-w-7xl mx-auto px-6">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-16 mb-16 md:mb-20">
            ...
            <div data-aos="fade-up">
                <a href="{{ route('home') }}" class="flex items-center gap-2 mb-6 md:mb-8 justify-center md:justify-start">
                    <div class="w-8 h-8 md:w-10 md:h-10 bg-yellow-400 rounded-xl flex items-center justify-center shadow-lg">
                    ...
                    <span class="text-xl md:text-2xl font-bold text-white tracking-tight">Lemans<span class="text-yellow-400"> Jaya</span></span>
                </a>
                ...
                <!-- <div class="flex gap-4">
                    @foreach(['facebook', 'instagram', 'whatsapp', 'youtube'] as $social)
                        <a href="#" class="w-12 h-12 bg-gray-800 rounded-2xl flex items-center justify-center text-gray-400 hover:bg-yellow-600 hover:text-white transition-all duration-300">
            ...
            <div data-aos="fade-up" data-aos-delay="100" class="text-center md:text-left">
                ...
                <ul class="grid grid-cols-2 md:grid-cols-1 gap-x-4 gap-y-3 md:gap-4">
                    @foreach(['Home' => 'home', 'Services' => 'services.index', 'Products' => 'products.index', 'Articles' => 'articles.index', 'Gallery' => 'gallery.index', 'Contact' => 'contact.index'] as $label => $route)
                        <li>
                            <a href="{{ route($route) }}" class="text-gray-400 hover:text-yellow-400 transition-colors flex items-center justify-center md:justify-start gap-2 group text-sm md:text-base font-medium">
                                <span class="w-1.5 h-1.5 bg-yellow-400 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"></span>
                                {{ $label }}
                            </a>
                        </li>
                    @endforeach
            ...
            <div data-aos="fade-up" data-aos-delay="300" class="text-center md:text-left">
                ...
                @if(session('success_newsletter'))
                    <div class="bg-yellow-600/20 text-yellow-400 p-4 rounded-xl mb-4 border border-yellow-600/30 text-[10px] md:text-xs font-bold">
                ...
                <form action="{{ route('newsletter.store') }}" method="POST" class="relative group">
                    @csrf
                    <input type="email" name="email" required placeholder="Email Address" class="w-full bg-gray-800 border-none rounded-2xl p-4 md:p-5 text-white focus:ring-2 focus:ring-yellow-400 transition-all font-bold text-xs md:text-sm pr-16 md:pr-20">
                    <button type="submit" class="absolute right-1.5 md:right-2 top-1.5 md:top-2 bottom-1.5 md:top-2 h-[calc(100%-12px)] md:h-[calc(100%-16px)] bg-yellow-400 text-white px-4 md:px-6 rounded-xl hover:bg-yellow-500 transition-colors shadow-lg shadow-yellow-600/20">
        ...
        <div class="border-t border-white/5 pt-8 md:pt-12 flex flex-col md:flex-row justify-between items-center gap-6 md:gap-8">
            <p class="text-gray-500 font-medium text-[10px] md:text-sm text-center md:text-left">
                &copy; {{ date('Y') }} <span class="text-white font-bold text-base md:text-lg">Lemans<span class="text-yellow-400"> Jaya</span></span>. All rights reserved.
            </p>

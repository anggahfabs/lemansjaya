...
<div class="pt-24 md:pt-40 pb-24 bg-gray-50 overflow-hidden relative">
    ...
    <div class="absolute top-0 right-0 w-1/2 h-1/2 bg-yellow-100/30 rounded-full blur-[120px] -translate-y-1/2 translate-x-1/2"></div>
    ...
    <div class="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div class="text-center mb-16 md:mb-24" data-aos="fade-down">
            <h2 class="text-yellow-400 font-bold uppercase mb-4 md:mb-6 text-[10px] md:text-sm tracking-widest">Contact Support</h2>
                    ...
                    <form action="{{ route('contact.store') }}" method="POST" class="space-y-6 md:space-y-8">
                        ...
                        <div class="grid md:grid-cols-2 gap-6 md:gap-8">
                            <div class="space-y-2 group">
                                <label class="block text-[10px] md:text-xs font-bold uppercase text-gray-400 ml-1 group-focus-within:text-yellow-400 transition-colors">Full Name</label>
                                <input type="text" name="name" class="w-full bg-gray-50 border-none px-6 py-4 md:py-5 rounded-xl md:rounded-2xl focus:ring-2 focus:ring-yellow-400 transition-all font-bold text-gray-900 text-sm md:text-base" placeholder="e.g. John Doe" required>
                            </div>
                            <div class="space-y-2 group">
                                <label class="block text-[10px] md:text-xs font-bold uppercase text-gray-400 ml-1 group-focus-within:text-yellow-400 transition-colors">Email Address</label>
                                <input type="email" name="email" class="w-full bg-gray-50 border-none px-6 py-4 md:py-5 rounded-xl md:rounded-2xl focus:ring-2 focus:ring-yellow-400 transition-all font-bold text-gray-900 text-sm md:text-base" placeholder="name@example.com" required>
                            </div>
                        </div>

                        <div class="space-y-2 group">
                            <label class="block text-[10px] md:text-xs font-bold uppercase text-gray-400 ml-1 group-focus-within:text-yellow-400 transition-colors">Phone Number</label>
                            <input type="text" name="phone" class="w-full bg-gray-50 border-none px-6 py-4 md:py-5 rounded-xl md:rounded-2xl focus:ring-2 focus:ring-yellow-400 transition-all font-bold text-gray-900 text-sm md:text-base" placeholder="e.g. 08123456789">
                        </div>

                        <div class="space-y-2 group">
                            <label class="block text-[10px] md:text-xs font-bold uppercase text-gray-400 ml-1 group-focus-within:text-yellow-400 transition-colors">Your Message</label>
                            <textarea name="message" rows="5" class="w-full bg-gray-50 border-none px-6 py-4 md:py-5 rounded-xl md:rounded-2xl focus:ring-2 focus:ring-yellow-400 transition-all font-bold text-gray-900 text-sm md:text-base" placeholder="What's on your mind?" required></textarea>
                        </div>

                        <button type="submit" class="w-full bg-yellow-300 text-white font-bold py-4 md:py-6 rounded-2xl md:rounded-3xl shadow-xl md:shadow-2xl shadow-yellow-200 hover:bg-yellow-600 hover:-translate-y-1 transition-all active:scale-[0.98] text-sm md:text-base flex items-center justify-center gap-3 md:gap-4">
                            Send Message
                ...
                <div class="bg-gray-900 rounded-3xl md:rounded-[4rem] overflow-hidden h-[400px] md:h-full shadow-lg md:shadow-2xl shadow-gray-400 border-4 md:border-8 border-white relative group">
                    ...
                    <div class="absolute bottom-6 md:bottom-12 left-6 md:left-12 right-6 md:right-12 bg-white/10 backdrop-blur-xl border border-white/20 p-6 md:p-8 rounded-3xl md:rounded-[2.5rem] shadow-2xl group-hover:bg-white group-hover:border-transparent transition-all duration-700">
                        <div class="flex items-center gap-4 md:gap-6">
                            <div class="w-12 h-12 md:w-16 md:h-16 bg-yellow-400 rounded-2xl flex items-center justify-center text-white shadow-xl flex-shrink-0">
